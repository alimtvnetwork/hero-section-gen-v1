...
**Scope audited:** `spec/01-rise-of-asia/01-homepage-hero-section/` (13-spec.md, SPEC-PART-2..6.md, 20-gap-analysis.md).
...
1. **Canonical vs SPEC contradiction (P0).** 20-gap-analysis.md overrides 13-spec.md on at least 7 visual rules (Top 5% vs 1%, italic single-line accent, no flowchart, no logo strip in hero, unframed stat chips, 3-segment carousel bottom-right, `min-h-[760px]`). 13-spec.md was never patched. An AI reading top-down builds the wrong design.
2. **Broken image reference (P0).** 13-spec.md §preamble and SPEC-PART-2 §16 point to `./01-hero.png`, which has been renamed to `01-hero-variant-archived.png`. Canonical is `00-hero-canonical.png`. AI loads the wrong source of truth.
3. **Stack drift (P1).** 13-spec.md uses Tailwind v3 config patterns + raw hex tokens; repo is Tailwind v4 with `@theme` in `src/styles.css` + `oklch()` — explicitly mandated by AGENTS rules. AI either ignores repo conventions or rewrites the design system.
4. **Naming conventions undefined (P1).** Events use `snake_case` (`hero_cta_click`), flags use `SCREAMING_SNAKE` (`HERO_SAFE_MODE`), files use `kebab-case` + ALL-CAPS mixed (`14-spec-part-2.md`, `20-gap-analysis.md`, `13-spec.md`). No convention doc says which to use where → bikeshedding + inconsistency at PR time.
...
6. **No TOC, no version, no acceptance map.** Sections jump from 0–11 (13-spec.md) to 12–24 (Part 2) to 45–64 (Parts 5/6). A reader can't tell what's MVP vs aspirational, what's shipped vs proposed.
...
|---|-------|---------------|------|
| 1 | Promote GAP-ANALYSIS deltas into 13-spec.md (single source of truth) | 13-spec.md | 25 min |
| 2 | Fix broken image refs (`01-hero.png` → `00-hero-canonical.png`) | 13-spec.md, 14-spec-part-2.md | 5 min |
| 3 | Reconcile "Top 1%" vs "Top 5%" everywhere | 13-spec.md, 15-spec-part-3.md, 16-spec-part-4.md | 10 min |
| 4 | Add `01-conventions.md` (naming: events, flags, files, CSS vars, components) | spec/01-rise-of-asia/01-homepage-hero-section/01-conventions.md | 20 min |
| 5 | Rewrite SPEC §2 tokens to Tailwind v4 + `oklch()` in `src/styles.css` style | 13-spec.md | 20 min |
| 6 | Drop archived elements still referenced (Part 2 §17 flowchart, §23 logo strip) | 14-spec-part-2.md | 10 min |
| 7 | Add `00-index.md` (TOC across all 7 spec files, MVP vs aspirational tags) | spec/01-rise-of-asia/01-homepage-hero-section/00-index.md | 25 min |
| 8 | Add version + changelog header to 13-spec.md (semver from Part 5 §45) | 13-spec.md | 10 min |
| 9 | Normalize time format (always `Xh Ym`, no mixed `75 min` / `1h 15m`) | SPEC-PART-2..6 | 15 min |
| 10 | Three.js fallback decision: kill, or add SSR/no-WebGL fallback rule | 13-spec.md §0, §11 | 15 min |
| 11 | Cross-ref Calendly (§3) ↔ Consent rendering (Part 6 §64) | 15-spec-part-3.md, 18-spec-part-6.md | 10 min |
| 12 | Replace raw HTML examples with React/TanStack equivalents | 13-spec.md examples block | 25 min |
| 13 | De-duplicate token definitions (SPEC §2 = source, Part 5 §57 = export only) | 17-spec-part-5.md | 10 min |
| 14 | Tag every Part 5/6 item as `[MVP]` / `[v1.1]` / `[aspirational]` | 17-spec-part-5.md, 18-spec-part-6.md | 15 min |
| 15 | File-name standardization (`13-spec.md` → `01-13-spec.md`, etc., or all-lowercase) | rename pass | 10 min |
...
- Changelog (Part 5 §45) references `21-changelog.md` — file does not exist.
