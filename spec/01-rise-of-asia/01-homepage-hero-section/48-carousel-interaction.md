...
- Resistance at edges: 0.3 rubber-band; never wraps (no infinite loop — see `05-hero-anatomy.md` §carousel).

## Keyboard (per APG carousel pattern — see `24-a11y.md`)
| Key | Action |
|---|---|
| ← / → | prev / next slide |
| Home / End | first / last |
...
## Auto-advance
- Interval: **6000ms** (paused under `prefers-reduced-motion`, see `23-motion.md`).
...
- `pointer-events: none` on slides while transitioning (320ms — see `23-motion.md`).

## Test fixtures
See `11-storybook-fixtures.md` §carousel + Playwright `carousel.swipe.spec.ts`.
