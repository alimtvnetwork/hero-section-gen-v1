...
- **Delegated:** Calendly modal (`<CtaCalendly />`), particle background (`<HeroParticles />`), logo strip (rendered **outside** `<Hero />` per `19-archived.md`).

## Forbidden

- Passing children. Hero is closed; configure via props.
- Mutating `engineers` after mount (use key change to re-init).
- Adding new props without bumping `MINOR` in `21-changelog.md`.