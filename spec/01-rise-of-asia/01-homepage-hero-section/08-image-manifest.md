...
3. **Real customer names forbidden** in Storybook fixtures (see `11-storybook-fixtures.md`). Manifest entries use real names only for production-bound assets.
...
`manifest.yaml` lives at `spec/01-rise-of-asia/01-homepage-hero-section/portraits/manifest.yaml` (created in Batch 3 step 21 alongside canonical snapshots).