...
- Flag: `HERO_PREVIEW_MODE` per `40-flags.md`.
...
- Editors cannot delete history; only platform admins (37-codeowners.md `@platform`) can prune.
...
- Set `HERO_SAFE_MODE=true` (see `40-flags.md`) — serves SSR baseline only, ignores CMS payload entirely.
