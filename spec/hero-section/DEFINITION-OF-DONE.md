...
|---|---|---|---|---|
| 1 | **Spec compliance** | @hero-owner | `00-INDEX.md` reading order followed; PR description lists touched files | All P0s in `PRIORITIES.md` ✅; no new P0 introduced. |
| 2 | **Visual diff = 0** | @design @qa | `snapshots/hero/diffs/` empty | Every baseline in `SNAPSHOTS.md` within `maxDiffPixelRatio` tolerance. |
| 3 | **Tokens** | @design | `src/styles.css` matches `TOKENS.md` verbatim | No raw hex in JSX; no Tailwind v3 patterns. |
| 4 | **Component contract** | @hero-owner | `Hero.types.ts` matches `COMPONENT-CONTRACT.md` | Type test (`tsc --noEmit`) + runtime invariants logged in dev. |
...
| 7 | **Motion + reduced-motion** | @design | `MOTION.md` table covered; S-1440-rm snapshot green | No `transition:` leaks under `prefers-reduced-motion`. |
| 8 | **A11y AAA** | @a11y | `axe-core` 0 violations; manual VoiceOver + NVDA scripts run | Contrast ≥ 7:1 for body text; carousel ARIA per `A11Y.md`. |
| 9 | **Performance budgets** | @perf | Lighthouse CI report attached | Every assertion in `BUDGETS.md` passes; LCP p75 ≤ 2.0s (RUM). |
...
| 11 | **Copy + legal** | @copy @legal | All changed strings approved per `COPY-OWNERS.md` | Legal-review keywords either absent or `@legal` approved. |
