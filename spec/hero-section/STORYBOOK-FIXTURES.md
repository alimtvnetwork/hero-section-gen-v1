...
Generated via `imagegen` per `IMAGE-MANIFEST.md`. Each fixture entry stores `seed` for reproducibility. **Never** use scraped social-media photos, stock-photo previews with watermarks, or real engineer headshots from `production-manifest.yaml`.

## Synthetic quotes

Drafted from a fixed pool (`__fixtures__/quotes.json`). Reviewed by copy owner (`COPY-OWNERS.md`) once; pool size: 12; rotation in stories deterministic via `fixtureSeed`.
...
New fixture additions require a 30-second eyeball check by `@privacy-reviewer` (CODEOWNERS rule). Synthetic names from the pool above are auto-approved.
